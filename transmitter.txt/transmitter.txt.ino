...
void loop() 
{
  ...
  { 
    if ((xval>360 && xval<416) && (yval>340 && yval<380)) //forward(transmitter tilted forward)
    {
      digitalWrite(ledPin,HIGH);
      send("f");
      
    }
    if ((xval>300 && xval<365) && (yval>365 && yval<400)) //backward(transmitter tilted backward)
    {
      digitalWrite(ledPin,HIGH);
      send("a");
      
    }
    if ((xval>350 && xval<380) && (yval>320 && yval<355)) //left(transmitter tilted to left)
    {
      digitalWrite(ledPin,HIGH);
      send("l");
     }
     if ((xval>360 && xval<390) && (yval>365 && yval<395))//right(transmitter tilted to right)
    {
      digitalWrite(ledPin,HIGH);
      send("r");
